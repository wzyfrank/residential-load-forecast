...
def genData_WC(user_load, n_train, temperature, humidity, pressure, dailycycle, weeklycycle, n_valid, n_lag, T, curr_day):
    ################## generate data ##########################################
    ...
    for train_day in range(curr_day - n_train - n_valid, curr_day - n_valid):
        ...
        for h in range(n_lag):
            ...
            X_train[row, :, h, 4] = dailycycle[train_day * T - n_lag * T + h * T: train_day * T - n_lag * T + (h+1) * Ｔ]
            X_train[row, :, h, 5] = weeklycycle[train_day * T - n_lag * T + h * T: train_day * T - n_lag * T + (h+1) * Ｔ]
        row += 1
    ...
    for valid_day in range(curr_day - n_valid, curr_day):
        ...
        for h in range(n_lag):
            ...
            X_valid[row, :, h, 4] = dailycycle[valid_day * T - n_lag * T + h * T: valid_day * T - n_lag * T + (h+1) * Ｔ]
            X_valid[row, :, h, 5] = weeklycycle[valid_day * T - n_lag * T + h * T: valid_day * T - n_lag * T + (h+1) * Ｔ]
        row += 1    
    ...
    for h in range(n_lag):
        ...
        X_test[row, :, h, 4] = dailycycle[curr_day * T - n_lag * T + h * T: curr_day * T - n_lag * T + (h+1) * Ｔ]
        X_test[row, :, h, 5] = weeklycycle[curr_day * T - n_lag * T + h * T: curr_day * T - n_lag * T + (h+1) * Ｔ]
...
def NN_forecast(sumLoad, temperature, humidity, pressure, dailycycle, weeklycycle, n_train, n_valid, n_lag, T):
    ############################ Iteration Parameter ##########################
    # maximum iteration
    Max_iter = 20000
    # stopping criteria
    ...
    #(l2, w2, b2) = add_layer(pool2_flat, T * n_lag * 4, N_neuron, activation_function=tf.nn.tanh)
    # output layer
    ...
    for curr_day in range(n_train + n_lag, n_days-1):
        ...
        sess.run(init) 
        # get data
        (X_train, y_train, X_valid, y_valid, X_test, y_test) = genData_WC(sumLoad, n_train, temperature, humidity, pressure, dailycycle, weeklycycle, n_valid, n_lag, T, curr_day)
        ...
        while (i < Max_iter):
            ...
            if(abs(last_l - l) < epsilon and l < 10):
                break
            else:
                last_l = l
                i = i+1
...
if __name__ == "__main__":
    ...
    (MAPE_avg, RMSPE_avg) = NN_forecast(sumLoad, temperature, humidity, pressure, dailycycle, weeklycycle, n_train, n_valid, n_lag, T)
